...
function init() {
    // Initialise l'API client avec la clé API
    gapi.client.init({
        apiKey: 'AIzaSyD1NCNddjv3JnzMhN2l5FcX7G2i7mwlbUc',
        // d'autres paramètres d'authentification...
    }).then(function() {
        // Appelle une fonction pour obtenir la liste des fichiers
        listFiles();
    });
}
...
function listFiles() {
    gapi.client.drive.files.list({
        ...
        if (files && files.length > 0) {
            // Il y a des fichiers dans le dossier
            for (var i = 0; i < files.length; i++) {
                var file = files[i];
                console.log('Nom du fichier:', file.name, 'ID du fichier:', file.id);
                
                // Vous pouvez ajouter le code pour afficher ces informations dans votre page HTML
                // Par exemple, ajouter ces informations à une liste ou à un élément de votre choix
            }
        } else {
            console.log('Aucun fichier trouvé dans le dossier MCO.');
        }
